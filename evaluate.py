...
def load_workouts(file_path: str) -> list[list[Workout]]:
    with open(file_path, "r") as f:
        return [
            [Workout(w) for w in ws.split(",")] for ws in f.read().split("\n") if ws
        ]


def get_results_for_scheduler(
    scheduler: Scheduler, evaluation_file_names
) -> dict[int, float]:
    results = {}
...
def main():
    ...
    print("---")
    print(
        f"- Number decreased (good):     {len([v for v in changes.values() if v < 0])}"
    )
    print(
        f"- Number unchanged (neutral):  {len([v for v in changes.values() if v == 0])}"
    )
    print(
        f"- Number increased (bad):      {len([v for v in changes.values() if v > 0])}"
    )
